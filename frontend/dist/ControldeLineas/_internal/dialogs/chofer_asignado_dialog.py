...
class ChoferAsignadoDialog(QDialog):
    ...
    def setup_ui(self, placa, marca_modelo):
        ...
        if self.mode == "add":
            ...
            self.txt_cedula.setValidator(QIntValidator(0, 999999999))
            ...
            def update_validator():
                if self.combo_campo.currentData() == "cedula":
                    self.input_nuevo.setValidator(QIntValidator(0, 999999999))
                else:
                    self.input_nuevo.setValidator(None)
            
            self.combo_campo.currentIndexChanged.connect(update_validator)
            update_validator() 
